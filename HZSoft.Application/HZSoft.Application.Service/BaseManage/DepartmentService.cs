...
namespace HZSoft.Application.Service.BaseManage
{
    ...
    public class DepartmentService : RepositoryFactory<DepartmentEntity>, IDepartmentService
    {
        ...
        {
            string strSql = "SELECT * FROM Base_Department where DeleteMark <> 1 ";
            if (!OperatorProvider.Provider.Current().IsSystem)
            {
                //string dataAutor = string.Format(OperatorProvider.Provider.Current().DataAuthorize.ReadAutorize, OperatorProvider.Provider.Current().UserId);
                //strSql += " and DepartmentId IN( select DepartmentId from Base_User where 1=1 and UserId in (" + dataAutor + ") GROUP BY DepartmentId )";
