    ...
    public class CustomerService : RepositoryFactory<CustomerEntity>, ICustomerService
    {
        ...
        public DataTable GetListJoin()
        {
            ...
            if (!OperatorProvider.Provider.Current().IsSystem && OperatorProvider.Provider.Current().Account != "admin")
            {
                ...
                sqlWhere += " And TraceUserId in (" + dataAutor + ")";
            }

            //string strSql = "Select Id,Mobile,Province,City,County,Address,FullName from ZZT_400Customer WHERE DeleteMark=0 AND EnabledMark=1 AND FullName IS NOT NULL AND FullName!='' " + sqlWhere 
            //    + " UNION Select Id,Mobile,Province,City,County,Address,FullName from ZZT_PDDCustomer WHERE DeleteMark=0 AND EnabledMark=1 AND FullName IS NOT NULL AND FullName!='' " + sqlWhere;

            string strSql = "SELECT Id,CompanyName FullName, CompanyAddress Address, Mobilephone Mobile FROM Ku_Company WHERE FollowState=3";
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable GetFormJoin(string keyValue)
        {
            //string strSql = "SELECT * FROM(Select Id,Mobile,Province,City,County,Address,FullName from ZZT_400Customer UNION Select Id,Mobile,Province,City,County,Address,FullName from ZZT_400Customer)t WHERE Id='" + keyValue+"'";
            string strSql = " SELECT Id,CompanyName FullName, CompanyAddress Address, Mobilephone Mobile FROM Ku_Company WHERE Id='" + keyValue + "'";
            return this.BaseRepository().FindTable(strSql.ToString());
        }
