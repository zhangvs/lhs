(function ($) {
    ...
    $.hzsofttab = {
        ...
        init: function () {
            $('.menuItem').on('click', $.hzsofttab.addTab);
            $('.menuTabs').on('click', '.menuTab i', $.hzsofttab.closeTab);
            $('.menuTabs').on('click', '.menuTab', $.hzsofttab.activeTab);
            $('.tabLeft').on('click', $.hzsofttab.scrollTabLeft);
            $('.tabRight').on('click', $.hzsofttab.scrollTabRight);
            $('.tabReload').on('click', $.hzsofttab.refreshTab);
        ...
        loadMenu: function () {
            ...
            $.each(data, function (i) {
                ...
                if (row.ParentId == "0") {
                    ...
                    _html += '</a>'
                    var childNodes = $.hzsoftindex.jsonWhere(data, function (v) { return v.ParentId == row.ModuleId });
                    if (childNodes.length > 0) {
                        _html += '<ul class="treeview-menu">';
                        $.each(childNodes, function (i) {
                            var subrow = childNodes[i];
                            var subchildNodes = $.hzsoftindex.jsonWhere(data, function (v) { return v.ParentId == subrow.ModuleId });
                            _html += '<li>';
